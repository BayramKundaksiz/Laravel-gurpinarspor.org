                ...
                <div class="navbar-nav mr-auto py-0">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu rounded-0 m-0">
                            ...
                            <a href="{{ route('tumalimsatim') }}" target="_blank" class="dropdown-item active">Alım
                                Satım</a>
                        </div>
                    </div>
    ...
    <div class="container-fluid">
        <div class="container">
            <nav class="breadcrumb bg-transparent m-0 p-0">
                ...
                <span class="breadcrumb-item active">Alım - Satımlar</span>
            </nav>
        </div>
    </div>
    <!-- Breadcrumb End -->
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="d-flex align-items-center justify-content-between bg-light py-2 px-4 mb-3">
                                <h3 class="m-0">Bütün Alım Satımlar</h3>
                                <a class="text-secondary font-weight-medium text-decoration-none" href=""></a>
                            </div>
                        </div>

                        @foreach ($tumalimsatim as $tumalimsatim)
                            <div class="col-lg-66">
                                <div class="position-relative mb-3">
                                    <a href="{{ route('alimsatimslug', $tumalimsatim->slug) }}">
                                        <img class="img-fluid w-100"
                                            src="alimsatimgorseller/{{ $tumalimsatim->urun_gorseli }}"
                                            style="object-fit: fill; height: 200px;">
                                        <div class="overlay position-relative bg-light">
                                    </a>
                                    <div class="mb-2" style="font-size: 14px;">
                                        <a href="{{ $tumalimsatim->slug }}">Satıcı İsmi: <span
                                                style="color: black">{{ $tumalimsatim->satici_ismi }}</span></a>
                                    </div>
                                    <span>İlan Tarihi: {{ $tumalimsatim->ilan_tarihi }}</span>
                                    <a class="h4"
                                        href="{{ $tumalimsatim->slug }}">{{ $tumalimsatim->urun_basligi }}</a>
                                    <p class="m-0">{{ Str::Limit($tumalimsatim->urun_aciklamasi) }}</p>
                                </div>
                            </div>
                    </div>
                    @endforeach
